...
| Cb por trecho | NBR 8800:2024, 5.4.2.3-a | FLT geral de seção duplamente simétrica | calculado com Mmax, MA, MB e MC do próprio trecho e limitado a 3,0; ainda não integrado ao motor de resistência da UI | `analysis.stability_segments.segment_moment_data` | `test_each_segment_uses_its_own_moments_cb_demand_and_resistance` e casos de vínculo | sem resultado assinado | PARTIAL_IMPLEMENTATION | não designado |
...
| uma mesa continuamente contida | NBR 8800:2024, 5.4.2.4 | quando exatamente uma mesa é contida continuamente | caminho separado e bloqueante até transcrição/revisão integral da expressão específica | `segment_moment_data` | `test_one_continuously_restrained_flange_uses_specific_blocking_path` | ausente | NORMATIVE_REVIEW_REQUIRED | não designado |
