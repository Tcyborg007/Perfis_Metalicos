...
def calcular_flecha_maxima(tipo_viga, L_cm, E, Ix, q_serv_kn_cm=0, p_serv_load=None):
    ...
    if p_serv_load:
        ...
        if tipo_viga=='Bi-apoiada':
            if a>=L/2: a,b=b,a
            if a<L: delta_p=(P*a*(L**2-a**2)**1.5)/(9*math.sqrt(3)*E*Ix*L) if (L**2-a**2)>=0 else 0
...
def gerar_memorial_completo(perfil_nome, perfil_tipo, resultados):
    return f"""<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Memorial - {perfil_nome}</title>{HTML_TEMPLATE_CSS}<script type="text/javascript" async src="https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.7/MathJax.js?config=TeX-MML-AM_CHTML"></script></head><body><div class="container"><h1>Memorial de Cálculo</h1><h2>Perfil: {perfil_nome} ({perfil_tipo})</h2><p style="text-align:center;font-style:italic;">Norma: <b>{Config.NOME_NORMA}</b></p>{resultados['resumo_html']}{resultados['passo_a_passo_html']}</div></body></html>"""
...
def load_data_from_local_file():
    ...
    except FileNotFoundError: st.error("Erro: 'perfis.xlsx' não encontrado."); return None
...
def _calcular_mrdx_flt(props,Lb,Cb,fy):
    ...
    if lambda_val<=lambda_p: Mrdx=Mp/Config.GAMMA_A1
    else:
        ...
        if lambda_val<=lambda_r: Mrdx=min((Cb/Config.GAMMA_A1)*(Mp-(Mp-Mr)*((lambda_val-lambda_p)/(lambda_r-lambda_p))),Mp/Config.GAMMA_A1)
        else:
...
def _calcular_mrdx_flm(props,fy):
    ...
    if lambda_val<=lambda_p: Mrdx=Mp/Config.GAMMA_A1
        ...
        if lambda_val<=lambda_r: Mrdx=(1/Config.GAMMA_A1)*(Mp-(Mp-Mr)*((lambda_val-lambda_p)/(lambda_r-lambda_p)))
        else: Mrdx=(0.69*Config.E_ACO*Wx/(lambda_val**2))/Config.GAMMA_A1 if lambda_val>0 else 0
    detalhes['Mrdx']=Mrdx
    return detalhes
...
def _calcular_vrd(props,fy):
    ...
    if lambda_val<=lambda_p: Vrd=Vpl/Config.GAMMA_A1
    else:
        lambda_r=Config.FATOR_LAMBDA_R_VRD*math.sqrt((kv*Config.E_ACO)/fy)
        if lambda_val<=lambda_r: Vrd=(lambda_p/lambda_val)*(Vpl/Config.GAMMA_A1) if lambda_val>0 else 0
        else: Vrd=(Config.FATOR_VRD_ELASTICO*(lambda_p/lambda_val)**2)*(Vpl/Config.GAMMA_A1) if lambda_val>0 else 0
    detalhes['Vrd']=Vrd
    return detalhes
...
def perform_all_checks(props, fy, Lb, Cb, L, Msd, Vsd, q_serv_kn_cm, p_serv_load, tipo_viga, input_mode, detalhado=False):
    ...
    nota_interacao = "Vsd ≤ 0.5*Vrd. Interação não crítica." if Vsd<=0.5*Vrd else "Vsd > 0.5*Vrd. Verificação de interação recomendada."
...
def build_summary_html(Msd,Vsd,res_flexao,res_cis,res_flecha):
    rows=[('Flexão (M)-ELU',f"{Msd/100:.2f} kNm",f"{res_flexao['Mrd']/100:.2f} kNm",res_flexao['eficiencia'],res_flexao['status']),('Cisalhamento (V)-ELU',f"{Vsd:.2f} kN",f"{res_cis['Vrd']:.2f} kN",res_cis['eficiencia'],res_cis['status'])]
    if res_flecha['status']!="N/A": rows.append(('Flecha (δ)-ELS',f"{res_flecha['flecha_max']:.2f} cm",f"≤ {res_flecha['flecha_limite']:.2f} cm",res_flecha['eficiencia'],res_flecha['status']))
...
def build_step_by_step_html(L,Msd,Vsd,res_flexao,res_cis,res_flecha,res_flt,res_flm,res_fla,res_vrd,input_mode):
    html = f"<h2>2. Esforços de Cálculo</h2><div class='formula-block'><p class='formula'>M<sub>sd</sub>={Msd/100:.2f} kNm</p><p class='formula'>V<sub>sd</sub>={Vsd:.2f} kN</p></div><h2>3. Verificações (ELU)</h2><h3>3.1. Resistência à Flexão (M<sub>rd</sub>)</h3>"
    ...
    html+=f"<h3>3.2. Resistência ao Cisalhamento (V<sub>rd</sub>)</h3>"; html+=_add_verification_details("Força Cortante (VRd)",res_vrd)
...
def _add_verification_details(title,details_dict):
    ...
    final_res,unit=details_dict.get('Mrdx',details_dict.get('Vrd',0)),'kNm' if'Mrdx'in details_dict else'kN'
    if unit=='kNm': final_res/=100
    html+=f"<h5>Resultado</h5><p class='final-result'>{title.split('(')[0].strip()} Resistente={final_res:.2f} {unit}</p></div>"
    return html
...
def run_detailed_analysis(df,perfil_nome,perfil_tipo_display,fy,Lb,Cb,L,Msd,Vsd,q_serv_kn_cm,p_serv_load,tipo_viga,input_mode):
    with st.spinner(f"Gerando análise para {perfil_nome}..."):
        try:
            ...
            st.success("Análise concluída!"); st.components.v1.html(html_content,height=800,scrolling=True)
...
def run_batch_analysis(all_sheets,fy,Lb,Cb,L,Msd,Vsd,q_serv_kn_cm,p_serv_load,tipo_viga,input_mode):
    all_results,total_perfis=[],sum(len(df) for df in all_sheets.values()); bar=st.progress(0,"Iniciando...")
    with st.spinner(f"Analisando {total_perfis} perfis..."):
        ...
        for i,(sheet_name,df) in enumerate(all_sheets.items()):
            for _,row in df.iterrows():
                ...
                try:
                    ...
                    all_results.append({'Tipo':sheet_name,'Perfil':row['Bitola (mm x kg/m)'],'Peso (kg/m)':props['Peso'],'Status':status,'Ef. Flexão (%)':res_flexao['eficiencia'],'Ef. Cisalh. (%)':res_cis['eficiencia'],'Ef. Flecha (%)':res_flecha.get('eficiencia',pd.NA)})
                except(ValueError,KeyError)as e: continue
            bar.progress(perfis_processados/total_perfis,f"Analisando: {Config.PROFILE_TYPE_MAP.get(sheet_name,sheet_name)}")
    bar.empty(); st.success(f"{len(all_results)} perfis analisados com sucesso.")
    if not all_results: st.warning("Nenhum perfil pôde ser analisado com os parâmetros atuais."); return
    df_results=pd.DataFrame(all_results); tabs=st.tabs([Config.PROFILE_TYPE_MAP.get(name,name)for name in all_sheets.keys()])
    def style_df(df_to_style):
        return df_to_style.style.format('{:.1f}',subset=[c for c in df_to_style if'%'in c],na_rep="N/A")
    for i,sheet_name in enumerate(all_sheets.keys()):
        with tabs[i]:
            df_type=df_results[df_results['Tipo']==sheet_name].drop(columns=['Tipo'])
            aprovados=df_type[df_type['Status']=='APROVADO'].sort_values(by='Peso (kg/m)')
            reprovados=df_type[df_type['Status']=='REPROVADO']
            if not aprovados.empty:
                st.subheader("🏆 Perfil Mais Leve Aprovado"); st.dataframe(style_df(aprovados.head(1)),use_container_width=True)
                with st.expander("Ver todos perfis aprovados"): st.dataframe(style_df(aprovados),use_container_width=True)
            else: st.info("Nenhum perfil aprovado nesta categoria.")
            if not reprovados.empty:
                with st.expander("Ver perfis reprovados"): st.dataframe(style_df(reprovados),use_container_width=True)
...
def main():
    ...
    with st.sidebar:
        ...
        L_cm=st.number_input("Comprimento (L, cm)",min_value=10.0,max_value=5000.0,value=500.0,step=10.0)
        st.header("2. Carregamento"); input_mode=st.radio("Modo de Entrada",("Calcular a partir de Cargas na Viga","Inserir Esforços Manualmente"),label_visibility="collapsed",horizontal=True)
        Msd,Vsd,q_serv_kn_cm,p_load_serv=0,0,0,None
        if input_mode=="Calcular a partir de Cargas na Viga":
            with st.container(border=True):
                st.subheader("Carga Distribuída (q)"); carga_area=st.number_input("Carga (serviço, kN/m²)",min_value=0.0,value=4.0,step=0.5); larg_inf=st.number_input("Largura de Influência (m)",min_value=0.0,value=5.0,step=0.5)
                st.subheader("Carga Pontual (P)")
                if st.checkbox("Adicionar Carga Pontual"):
                    p_serv_kn=st.number_input("Valor P (serviço, kN)",min_value=0.0,value=10.0); p_pos_cm=st.number_input("Posição P (x, cm)",min_value=0.0,max_value=L_cm,value=L_cm/2); p_load_serv=(p_serv_kn,p_pos_cm)
                gamma_f=st.number_input("Coef. Majoração (γf)",min_value=1.0,max_value=2.0,value=1.4,step=0.1)
                q_serv_kn_cm=(carga_area*larg_inf)/100.0; p_load_ult=(p_load_serv[0]*gamma_f,p_load_serv[1]) if p_load_serv else None
                Msd,Vsd=calcular_esforcos_viga(tipo_viga,L_cm,q_serv_kn_cm*gamma_f,p_load_ult)
        else:
            with st.container(border=True):
                st.warning("Verificação de flecha (ELS) não realizada no modo manual."); msd_input=st.number_input("Msd (kNm)",min_value=0.0,value=100.0); Vsd=st.number_input("Vsd (kN)",min_value=0.0,value=50.0); Msd=msd_input*100
        st.header("3. Parâmetros do Aço"); fy_aco=st.number_input("Tensão Escoamento (fy, kN/cm²)",min_value=20.0,max_value=50.0,value=34.5,step=0.5)
        Lb_projeto=st.number_input("Comp. Destravado (Lb, cm)",min_value=10.0,max_value=L_cm,value=L_cm,step=10.0); Cb_projeto=st.number_input("Fator Modificação (Cb)",min_value=1.0,max_value=3.0,value=1.1,step=0.1)
        st.header("4. Modo de Análise"); analysis_mode=st.radio("Modo de Análise",("Memorial Detalhado de um Perfil","Análise em Lote com Otimização"),label_visibility="collapsed",horizontal=True)

    if analysis_mode=="Memorial Detalhado de um Perfil":
        st.header("🔍 Memorial de Cálculo Detalhado")
        col1,col2=st.columns(2)
        with col1: selected_display_name=st.selectbox("Tipo de Perfil",tuple(Config.PROFILE_TYPE_MAP.values()))
        sheet_name={v:k for k,v in Config.PROFILE_TYPE_MAP.items()}[selected_display_name]
        df_selecionado=all_sheets[sheet_name]
        with col2: perfil_selecionado_nome=st.selectbox("Perfil Específico",df_selecionado['Bitola (mm x kg/m)'])
        if st.button("Gerar Memorial Completo",type="primary",use_container_width=True):
            run_detailed_analysis(df_selecionado,perfil_selecionado_nome,selected_display_name,fy_aco,Lb_projeto,Cb_projeto,L_cm,Msd,Vsd,q_serv_kn_cm,p_serv_load,tipo_viga,input_mode)
    elif analysis_mode=="Análise em Lote com Otimização":
        st.header("📊 Análise Otimizada por Categoria")
        if st.button("Iniciar Análise Otimizada",type="primary",use_container_width=True):
            run_batch_analysis(all_sheets,fy_aco,Lb_projeto,Cb_projeto,L_cm,Msd,Vsd,q_serv_kn_cm,p_serv_load,tipo_viga,input_mode)
